...
class WXSwipeTableViewCell: UITableViewCell {
    ...
    func gestureHandle(gesture:UIPanGestureRecognizer) {
        ...
        case .Began:()
        case .Changed:
            moveContentView(offset: point.x.d, animated: false)
            let (swipeState, direction) = findSwipeState(offset: point.x.d);
    ...
    func animateSwipe(#direction:SwipeDirection, completed: ()->() = {}) {
        let finalX = direction == .DirectionLeft ? -self.contentView.frame.width.d : self.contentView.frame.width.d
    ...
    func moveContentView(#offset:Double, animated:Bool, completed: ()->() = {}) {
        var block = { ()->() in
            ...
            self.contentView.frame = CGRectMake(offset.f, 0, frame.size.width, frame.size.height);
        }

        if animated {

//            UIView.animateWithDuration(0.6, delay: 0.1, usingSpringWithDamping: 0.65, initialSpringVelocity: 1, options: UIViewAnimationOptions.CurveEaseOut, animations: block, completion: {(completion:Bool) in
//                completed();
//            })


            UIView.animateWithDuration(0.4, delay: 0, options: UIViewAnimationOptions.CurveEaseIn, animations: block, completion: {(completion:Bool) in
                completed();
                })
...
//            });
    ...
    override func gestureRecognizerShouldBegin(gestureRecognizer: UIGestureRecognizer!) -> Bool {
        ...
        return fabs(translation.y.d) < fabs(translation.x.d)
    }
